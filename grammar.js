...
module.exports = grammar({
  ...
  rules: {
    ...
    _native_struct_declaration: $ => seq(
      optional($.attributes),
      repeat($._modifier),
      'native',
      'struct',
    ...
    _regular_struct_declaration: $ => seq(
      optional($.attributes),
      repeat($._modifier),
      'struct',
      field('name', $.identifier),
    ...
    _native_function_declaration: $ => seq(
      optional($.attributes),
      repeat($._modifier),
      'native',
      repeat($._modifier),
    ...
    _regular_function_declaration: $ => seq(
      optional($.attributes),
      repeat($._modifier),
      'fun',
      field('name', $.identifier),
    ...
    name_access_chain: $ => prec.left(seq(
    ...
    range_expression: $ => prec(PREC.RANGE + 1, seq(
    ...
    abort_expression: $ => prec.right(seq(
      'abort',
      $._expression,
    )),
    ...
    assign_expression: $ => prec.right(PREC.ASSIGN, seq(
    ...
    move_expression: $ => prec(PREC.UNARY, seq(
      'move',
      $._unary_expression,
    )),

    copy_expression: $ => prec(PREC.UNARY, seq(
      'copy',
      $._unary_expression,
    )),
    ...
    call_expression: $ => choice(
      prec.dynamic(2, prec(PREC.CALL, seq(
    ...
    macro_call_expression: $ => prec(PREC.CALL, seq(
      field('macro', alias(token(seq(/[a-zA-Z_][a-zA-Z0-9_]*/, '!')), $.macro_identifier)),
      field('arguments', $.arg_list),
    )),
    ...
    indirect_call_expression: $ => prec.left(PREC.CALL, seq(
      field('function', $.parenthesized_expression),
      field('arguments', $.arg_list),
    )),
    ...
    cast_expression: $ => prec.left(PREC.AS, seq(
    ...
    is_expression: $ => prec.left(PREC.EQ, seq(
      field('expr', $._expression),
      'is',
      sepBy1('|', $.name_access_chain),
    )),
    ...
    vector_expression: $ => prec(PREC.CALL + 1, seq(
    ...
    bind_field: $ => seq(
      field('field', $.identifier),
      optional(seq(':', field('bind', $._bind))),
    ),

    bind_positional_unpack: $ => seq(
      $.name_access_chain,
      optional($.type_arguments),
      '(',
    ...
    quantifier_expression: $ => prec.right(choice(
      seq(
    ...
    spec_condition: $ => seq(
      choice(
        'assert', 'assume', 'decreases', 'ensures', 'succeeds_if',
        seq('requires', optional('module')),
        'aborts_if', 'aborts_with', 'modifies',
      ),
      optional($.condition_properties),
    ...
    condition_properties: $ => seq(
      '[',
      commaSep($.spec_property),
      ']',
    ),

    spec_property: $ => seq(
      $.identifier,
      optional(seq('=', $._literal_value)),
    ),
