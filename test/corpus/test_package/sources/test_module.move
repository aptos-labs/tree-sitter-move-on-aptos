...
module module_addr::test_module {
        ...
        if (escrow_address == &@0x0) {
            ...
            *escrow_address = object::address_from_constructor_ref(&constructor_ref);
            ...
            match (escrow) {
                ...
                TimeUnlock { unlock_secs, original_owner: _, delete_ref: _ } => {
    ...
    public entry fun return_user_funds(
        ...
        let original_owner = match (&Escrow[escrow_address]) {
            Escrow::Simple { original_owner: owner, delete_ref: _ } => {
                *owner
            }
            Escrow::TimeUnlock { original_owner: owner,  unlock_secs: _, delete_ref: _} => {
    ...
    public entry fun return_my_funds(
        ...
        // Determine original owner, and any conditions on returning
        let original_owner = match (&Escrow[escrow_address]) {
            Escrow::Simple { original_owner, delete_ref: _ } => {
                *original_owner
            }
            Escrow::TimeUnlock { original_owner, unlock_secs, delete_ref: _ } => {
    ...
    package inline fun delete_escrow(self: &mut Lockup, lockup_key: EscrowKey) {
        ...
        let delete_ref = match (move_from<Escrow>(escrow_addr)) {
            Escrow::Simple { delete_ref, original_owner: _ } => {
                delete_ref
            }
            Escrow::TimeUnlock { delete_ref, original_owner: _, unlock_secs: _ } => {
                delete_ref
            }
        };
    ...
    package fun remaining_escrow_time(
        ...
        if (lockup.escrows.contains(escrow_key)) {
            ...
            let remaining_secs = match (&Escrow[*escrow_address]) {
                Simple { original_owner: _, delete_ref: _ } => { 0 }
                TimeUnlock { unlock_secs, original_owner: _, delete_ref: _ } => {
